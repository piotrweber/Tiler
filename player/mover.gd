...
@export_range(1.0,5.0,0.5) var dash_speed_multiplier = 2.0
@export_range(0.05,1.0,0.05) var dash_duration = 0.2
...
## The time interval in which player will be able to jump
...
func move_x(current_x: float, horizontal_input: float, delta: float) -> float:
	...
	var dash_mult = dash_speed_multiplier if is_dashing else 1.0
...
func get_gravity(horizontal_input: float) -> float:
	...
	if parent.is_on_wall_only() and parent.velocity.y > 0 and horizontal_input != 0:
		return wall_gravity
	# Jump cut: ramp up gravity when jump is released while still ascending
